...
import griffe_public_wildcard_imports
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("griffe_public_wildcard_imports")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["griffe_public_wildcard_imports._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["griffe_public_wildcard_imports"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `griffe_public_wildcard_imports`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in griffe_public_wildcard_imports.__all__
        or not hasattr(griffe_public_wildcard_imports, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in griffe_public_wildcard_imports.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("griffe_public_wildcard_imports")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (
                item.name == "griffe_public_wildcard_imports" or item.name.startswith("griffe_public_wildcard_imports.")
            )
        ):
